...
def clean(df_taxi: pd.DataFrame) -> pd.DataFrame:
    ...
    print(f"{len(df_rm_zero)} rows saved")
    return df_rm_zero
...
def write_local(df: pd.DataFrame, fpath: Path) -> Path:
    ...
    if not fpath.parent.exists():
        print(f"creating data directory {fpath.parent} first")
        fpath.parent.mkdir()
...
@task()
def upload_gcs(block_name: str, path: Path) -> None:
    """Upload the local parquet file to GCS"""
    gcs_block = GcsBucket.load(block_name)
    gcs_path = Path(path.parts[-2]) / path.parts[-1]
